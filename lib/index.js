...
export function convertRawToGlobalVariables(rawVariables, globalVariables){
	reducerReadyRawVariables = rawVariables.unshift(globalVariables);
	updatedGlobalVariables = reducerReadyRawVariables.reduce( (updatingGlobalVariables, rawVariable) => {
		updatingGlobalVariables.merge(convertRawVariableToObject(rawVariable));
	});
}

export function convertRawVariableToObject(rawVariable){
	const cleanRawVariable 	= rawVariable.slice(1, -1);
	const rawVariableKey 	= getRawVariableKey(rawVariable);
	const rawVariableValue 	= getRawVariableValue(rawVariable);
