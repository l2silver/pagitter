...
export function run(file = 'naetorp.js'){
				...
				readFile(file, 'utf8')
				.then((contents)=>{
					const codes = splitFile(contents).get('codes')
					stateHistory = codes.unshift(List([Map()])).reduce((state, code, index)=>{
						actions.push(state.last().set('code', code)).reduce((currentState, action)=>{
...
export function updateFilename(code, globalVariables){
	const filename = getFilename(code);
	return globalVariables.set('eTrfilenameETr', filename)
}	
...
export function transformContent(content, globalVariables){
	return content.replace(transformRegExp(globalVariables), (matched)=>{
