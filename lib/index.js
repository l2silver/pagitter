...
export function splitContent(file){
	...
	const contents = List(file.split(/\/\*_(?:.|\s)+?\*\//));
...
export const transformContent = Promise.method((state)=>{
	if(state.get('globalVariables').toList().size > 0){
		return state.set('content', 
			trimContent(transform(state.get('content'), state.get('globalVariables')))
		);
	}
	return state.update('content', content=>trimContent(content));
});
