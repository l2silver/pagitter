...
export const reverse = Promise.method((state)=>{
	reverseTransformContent(state).then((nextState)=>{
		if(nextState.last){
...
export function writePagitter(state){
	return writeFile(process.cwd()+'/pagitter.js', state.get('reverseContent'));
}
...
export const reverseTransformContent = Promise.method((state)=>{
	...
	return nextState.updateIn(['reverseContent'], content=>content+nextState.get('code')+newContent);
});
