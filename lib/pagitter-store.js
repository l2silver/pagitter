...
export function checkEndStores(state){
	if(state.hasIn(['globalVariables', 'pagitterStoresEnd']) || state.has('last')){
...
export const reverseTransformContent = Promise.method((state)=>{
	...
	.then((nextNextState)=>{
		const newContent = nextNextState.get('pagitterStoresExternalContent')
		.replace(reverseTransformRegExp(nextNextState.get('reverseGlobalVariables')), (matched)=>{
			const switchedMatch = nextNextState.getIn(['reverseGlobalVariables', matched]);
